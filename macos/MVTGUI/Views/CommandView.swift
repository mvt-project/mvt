...
struct CommandView: View {
    ...
    var body: some View {
        ...
        .navigationSubtitle(command.tool == .common ? "mvt" : command.tool.rawValue)
    }
    ...
    private var fields: some View {
        ...
        if options.contains(.output) || options.contains(.iocs) {
            Section("Analysis") {
                ...
                if options.contains(.iocs) {
                    IndicatorList(files: $form.iocFiles)
                }
        ...
        if command == .downloadIOCs {
            Section("Indicators folder") {
                LabeledContent("Stored in") {
                    HStack {
                        Text((MVTEnvironment.indicatorsDirectory.path as NSString).abbreviatingWithTildeInPath)
                            .textSelection(.enabled)
                        Button("Reveal") {
                            let url = MVTEnvironment.indicatorsDirectory
                ...
                Text("Every check loads these automatically, in addition to any indicator files you add. Public indicators alone cannot prove a device is clean.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var runButtonTitle: String {
        ...
        case .downloadIOCs: return "Download"
...
/// Editable list of STIX2 indicator files passed with --iocs.
private struct IndicatorList: View {
    @Binding var files: [String]

    var body: some View {
        LabeledContent("Indicator files") {
            VStack(alignment: .trailing, spacing: 4) {
                if files.isEmpty {
                    Text("Downloaded indicators only")
                        .foregroundStyle(.secondary)
                } else {
                ...
                Button("Add STIX2 File…") {
                    let panel = NSOpenPanel()
                    panel.canChooseFiles = true
                    panel.canChooseDirectories = false
                    panel.allowsMultipleSelection = true
                    if panel.runModal() == .OK {
                        for url in panel.urls where !files.contains(url.path) {
