...
final class ProcessRunner: ObservableObject, @unchecked Sendable {
    ...
    /// Set when the user presses Stop, so the result reads "Stopped" rather
    /// than as an error (Python exits with status 130 on SIGINT).
    ...
    private func start(
        ...
        Thread.detachNewThread { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                if self.stopRequested || process.terminationReason == .uncaughtSignal {
