...
struct Invocation {
    ...
    var onlyPassedIndicators = false
...
/// Which indicators a check compares the data against.
enum IndicatorMode: String, CaseIterable, Identifiable {
    /// MVT's default: every downloaded indicator file, plus any extra files.
    case allDownloaded
    /// Only the downloaded files picked in the form, plus any extra files.
    case picked
...
final class CommandForm: ObservableObject {
    ...
    @Published var indicatorMode = IndicatorMode.allDownloaded
    /// Downloaded indicator files picked for this check (used in `.picked` mode).
    @Published var pickedIndicators: Set<String> = []
    /// Extra STIX2 files chosen from anywhere on disk.
    ...
    func validationError(listModulesOnly: Bool = false) -> String? {
        ...
        if has(.iocs) {
            for ioc in indicatorFiles where !fm.fileExists(atPath: ioc) {
    ...
    var indicatorFiles: [String] {
        let picked = indicatorMode == .picked ? pickedIndicators.sorted() : []
        return picked + iocFiles.filter { !picked.contains($0) }
    }
    ...
    func invocation(listModulesOnly: Bool = false) -> Invocation {
        ...
        if has(.iocs) {
            for ioc in indicatorFiles { args += ["--iocs", ioc] }
        }
        ...
        var invocation = Invocation(tool: command.tool, arguments: args, environment: env)
        invocation.onlyPassedIndicators = has(.iocs) && indicatorMode == .picked
        return invocation
    }
}
